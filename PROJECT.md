...
| 27. Docs refresh | ◐ in progress | 2026-09-17 — PLAN.md rewritten as the UI-refocus roadmap; PROJECT.md synced; README trim pending |
| 28–31. UI refocus | ☐ planned | Two-tier nav (Pipeline primary + Tools group), `PipelineView` composing the existing FocusQueue/ReviewDeck/ApprovedDeck, split the 62 KB `JobsView`. See **PLAN.md** |

**In progress now (2026-09-17):** Refocusing the tool on the **triage → tailor → apply** daily loop (see PLAN.md M26–M31). M26 repo cleanup done (fresh `output/resumes`, LinkedIn outreach wired); M27 docs in progress; M28–M31 is the UI re-hierarchy (funnel becomes the primary surface, all other features demote to a "Tools" group — nothing removed).
