...
│  │  ├─ daily_reminder.py       # daily apply/referral/follow-up nudge email (M35)
...
│        └─ components/
│           ├─ Sidebar.jsx       # two-tier nav: Workspace (Pipeline) + Tools group (M28)
│           ├─ PipelineView.jsx (M29) — daily surface; wraps JobsView.jsx + pipeline/ split (M30); AnalyticsView.jsx
           ├─ lib/linkedinMessage.js — referral-DM generator, mirrors linkedin_outreach.py (M32)
...
│           ├─ OutreachView.jsx  # Recruiters + email composer + LinkedIn cockpit + filters (M16/M32/M33)
│           ├─ SettingsView.jsx  # Settings (UI only until M7)
│           ├─ JobDrawer.jsx     # Job detail + Reach out / Prep + H-1B badge (M31/M34)
...
├─ scripts/                      # ops utilities (enrich, cleanup_stale_jobs, refresh_h1b_sponsors, linkedin_outreach, sync_to_turso)
│  └─ linkedin_outreach.py       # rank LinkedIn Connections.csv → outreach shortlist (H-1B sponsors + live apps); `make outreach-linkedin`
├─ data/                         # gitignored — raw personal exports (LinkedIn archive: only Connections.csv is used)
├─ output/resumes/               # LLM-generated PDFs (gitignored as of M26 — regenerated per run, not committed)
...
| 11. Production Hardening | ✅ done (scope revised) | 2026-07-19 — /api/health + startup logging + PDF-path resolution done. The only open item was live **Render** deploy verification, which is **moot** — Render was dropped 2026-07-21 (local-only). Concurrency hardening later added (per-thread DB connections, see 2026-09-17 log) |
...
| 20. Inbox Ingestion (auto status) | ✅ done | 2026-09-05 — read recruiter replies over IMAP, classify (applied/interview/oa/rejection/offer), match to one active job, and advance status (fires notifications). Precision-biased: ambiguous mail is left alone. `--scan-inbox`; shared `pipeline/notifications.py`. **`--ingest`** also reads the "job apps" folder and reconstructs the full funnel from email (`application_parser.py` extracts company/role; one record per company; verified on the real mailbox: 303 apps, 4 interview / 2 OA / 38 rejected). Nested IMAP folders with spaces handled. **Hardened (2026-09-08):** confirmation-subject-authoritative classifier kills false interview/offer detections; interview detection made precision-first; email content attached to the matched job |
...
| 26. Repo cleanup | ✅ done | 2026-09-17 — untracked+deleted 861 generated PDFs under `output/resumes/` (now gitignored, tailoring starts fresh); gitignore `output/outreach/` + `data/`; LinkedIn archive wired (only `Connections.csv` → gitignored `data/linkedin/`), `scripts/linkedin_outreach.py` defaults to it + `make outreach-linkedin`, verified (2373 conns → 276 shortlist). Remaining: delete stale one-off scripts, rotate old `dist` bundles |
| 27. Docs refresh | ✅ done | 2026-09-17 — PLAN.md rewritten as the UI-refocus roadmap; PROJECT.md synced; README trimmed + outreach workflow added |
| 28. Two-tier nav | ✅ done | 2026-09-17 — Sidebar split into a prominent Workspace group + demoted Tools group; stale "Summer 2026" copy fixed |
| 29. Pipeline merge | ✅ done | 2026-09-17 — Dashboard + Jobs merged into one "Pipeline" surface; follow-ups folded into its header; DashboardView deleted |
| 30. Split JobsView | ✅ done | 2026-09-17 — 62 KB `JobsView` → 18.7 KB + `components/pipeline/` (ImportModal/FocusQueue/ReviewDeck/ApprovedDeck/constants); byte-identical bundle |
| 31. Polish + verify | ✅ done | 2026-09-17 — inline Prep shortcut (deep-links Interview Prep to the job); full in-browser click-through, zero console/server errors |
| 26b. /api/stats concurrency fix | ✅ done | 2026-09-17 — per-thread sqlite connections; killed the intermittent 500 (see decision log) |
| 32. LinkedIn referral outreach | ✅ done | 2026-09-21 — worklist DMs + in-app cockpit (Copy/Open/Mark-as-messaged); `POST /api/recruiters/{id}/log-linkedin` logs hand-sent DMs into the follow-up system; nothing auto-sent |
| 33. Outreach & pipeline filters | ✅ done | 2026-09-21 — Outreach: company + search + messaged filter; Jobs: "H-1B sponsors only" toggle |
| 34. H-1B sponsor visibility | ✅ done | 2026-09-21 — `/api/jobs` annotates `known_sponsor`/`sponsor_excluded`; H-1B badge on rows + drawer |
| 35. Daily reminder email | ✅ done | 2026-09-21 — `--remind` + `daily_reminder.yml` (daily ~9 AM Central): apply/referral/follow-up nudge. Opt-in on SMTP secrets. Cloud reads Turso (jobs section reliable; referral/follow-up need local state synced) |
| 36. Local→Turso sync + auto-prune | ✅ done | 2026-09-21 — `scripts/sync_to_turso.py` (`make sync-turso`) pushes tracked jobs/recruiters/outreach up so the cloud reminder is complete; daily stale-job pruning added to `daily_tailor.yml` (was monthly); `make tidy` on demand |
| 37. Inbox sync GitHub flow | ✅ done | 2026-09-21 — `inbox_sync.yml` (daily 13:00 UTC) runs `--scan-inbox --ingest` so externally-applied jobs (LinkedIn etc., via confirmation emails) auto-track into Turso + appear in the daily reminder. Needs GH secret `IMAP_MAILBOX=INBOX.job apps.reverts` |

**Latest (2026-09-21):** Added a **daily reminder email** (`--remind` + `daily_reminder.yml`, ~9 AM Central), a **local→Turso sync** (`make sync-turso`) so the cloud reminder sees your real progress, and **daily auto-pruning** of old un-applied jobs — on top of the day's other job-search enhancements. **LinkedIn referral cockpit** — for 1st-degree connections the Outreach tab generates a personalized referral DM you Copy / Open-profile / Mark-as-messaged; "messaged" is logged (`/log-linkedin`) so it counts as sent and gets a 7-day follow-up (nothing auto-sends). **Filters** — company + search + messaged in Outreach, "H-1B sponsors only" in the Jobs view. **Sponsor visibility** — every job is tagged `known_sponsor` and shows an H-1B badge, so F-1-relevant roles are prioritizable at a glance. All milestones M1–M34 done (M11 revised — Render moot). 241 tests pass; verified end-to-end in-browser; pushed to master.

**Prior (2026-09-17):** UI refocus **complete** (M26–M31) — single **Pipeline** surface, Tools group, `JobsView` split 62 KB → 18.7 KB, inline Prep shortcut, intermittent `/api/stats` 500 fixed.

**Prior autonomous run (2026-09-05):** **M6 auto-apply finished** (all ATS handlers) + **six new features shipped (M20–M25):** inbox ingestion → auto status-tracking, ATS keyword match, application follow-ups, weekly digest, cross-source dedup, and funnel analytics. Email generalized to any SMTP mailbox; test suite made hermetic; `datetime.utcnow()` deprecations removed. Full tree flake8-clean; **201 pytest pass**; web build green + verified in-browser (ATS panel, funnel/sponsor cards, follow-up banner). M11's Render box is moot (Render dropped — see 2026-07-21).
**Next up (needs Ishani):** (1) put your mailbox password in `.env` as `SMTP_PASSWORD` (host/port/user already set) to enable outreach sends, notifications, **inbox scanning** (`--scan-inbox`), and the digest — all wired, all no-op safely without it; (2) run `python main.py --apply --dry-run` on a few approved jobs to sanity-check ATS auto-fill selectors, then `--apply` for real; (3) run `python main.py --dedup` to collapse the duplicate listings (e.g. the repeated "Data & AI Engineer @ Arizent"); (4) warm-referral outreach to ex-AWS/Google/MS/Uber contacts, timed to each company's window — start from `make outreach-linkedin` (276-person ranked shortlist, 64 at interview-stage companies); (5) optionally add the SMTP secrets to GitHub to enable the weekly-digest workflow.

---

## Decision log

- 2026-09-21 — Inbox-sync GitHub flow for externally-applied jobs — Ishani applies to jobs outside this tool (LinkedIn etc.) and wanted those in the daily email. Rather than per-platform scraping, reused the M20 email-ingest engine (`--scan-inbox --ingest`): application *confirmation emails* arrive regardless of where she applied, so a daily `inbox_sync.yml` (13:00 UTC, before the 14:00 reminder) reconstructs applications from the "job apps" folder into Turso, feeding the reminder + follow-ups. Read-only on the mailbox, precision-biased, opt-in on secrets (IMAP creds default to SMTP). Her responses folder is non-default (`INBOX.job apps.reverts`), so GHA needs `IMAP_MAILBOX` set to it (applied folder default `INBOX.job apps` already matches). Historical LinkedIn apps (in the export's `Job Applications.csv`) are a separate one-time backfill, left as backlog since the email flow covers new applications.
- 2026-09-21 — Local→Turso sync + daily auto-pruning — resolves the cloud-reminder data gap (see prior entry): `scripts/sync_to_turso.py` mirrors local personal state (tracked jobs upserted by url, recruiters deduped, outreach mirrored with recruiter_id remapped and job_id dropped) up to Turso, so GHA's referral + follow-up sections populate. It never deletes Turso's scraped `new` jobs. The clean end state is convergence: once TURSO_* is in `.env`, the local app (which `load_dotenv()`s at import) reads Turso too, so there's a single DB and the sync becomes a no-op — but the sync also supports the keep-working-locally + periodic-push model. Separately, per Ishani's "keep deleting very old un-applied jobs", the existing `cleanup_stale_jobs.py` (new/queued, 30d+) now runs **daily** in `daily_tailor.yml` instead of monthly, plus `make tidy` on demand. Sync tested with a two-SQLite stand-in (no Turso reachable from the dev env).
- 2026-09-21 — Daily reminder email via GitHub Actions (Ishani's choice) — a `--remind` command builds a short daily apply/referral/follow-up nudge (`pipeline/daily_reminder.py`), scheduled by `daily_reminder.yml` at 14:00 UTC (~9 AM Central). Chose GHA over local launchd for always-on delivery even when her Mac is off. **Known tradeoff:** GHA reads Turso (daily-scraped jobs), so the Apply/sponsor-jobs section is always fresh, but her applied statuses + LinkedIn recruiters live in the local DB — so the referral/follow-up sections only populate in cloud mode once that state is synced to Turso. Follow-up: sync local personal state to Turso (or converge on one DB). Opt-in on SMTP secrets in GitHub (still needs to be added).
- 2026-09-21 — LinkedIn is the channel for warm connections; email is for cold outreach — the 276-person shortlist is all 1st-degree connections, so DMs land in their real inbox with no email-discovery/deliverability problem. Built an in-app **LinkedIn cockpit** (compose → Copy/Open profile → Mark as messaged) instead of forcing them through the email sender. **Nothing is auto-sent**: automating DMs against the user's LinkedIn account risks bans + violates ToS, and outbound messaging needs a human anyway — so the tool composes + tracks, the user sends by hand. "Mark as messaged" records a `linkedin`-type outreach row (`/log-linkedin`) so hand-sent DMs still get a 7-day follow-up. **Country filter deferred** — LinkedIn's `Connections.csv` carries no location and `recruiters` has no such column; a country filter would filter on nothing, so it's backlog (needs a manual/editable field) rather than a broken control.
- 2026-09-21 — Surface H-1B sponsor status on jobs — the whole search is F-1-constrained, so sponsor-friendliness is the highest-signal prioritization axis. `/api/jobs` now annotates each job `known_sponsor`/`sponsor_excluded` (computed from the curated list via `pipeline.sponsorship`, not stored), the list/drawer show an **H-1B** badge, and the Jobs view has an "H-1B sponsors only" toggle (New tab 287 → 51). Computed per-request (cached frozenset, fast) rather than denormalized onto the jobs table, so the sponsor list stays the single source of truth.
- 2026-09-17 — Fixed intermittent `/api/stats` 500 (concurrency) — the API shared one module-level sqlite3 connection across FastAPI's threadpool; a page load fires ~7 queries in parallel, and concurrent `execute()` on a single connection crosses cursor state, so `get_stats` built a `sqlite3.Row` against a mismatched column description → `IndexError: tuple index out of range`. Fix: `db()` now hands each thread its own sqlite connection (schema created once via `init_db`, then lightweight `tracker.connect()` per thread; WAL keeps committed writes visible across connections). The Turso path stays a single shared HTTP wrapper (each execute is an independent request). Tests inject via `M._conn`, which `db()` still honors. Verified: 60 concurrent `/api/stats` + 80 mixed requests all 200, zero server errors; 3 regression tests added (237 total).
- 2026-09-17 — Refocus on the apply loop + repo cleanup (M26–M31) — the pipeline is feature-complete but hard to use daily; seven equal nav items and a 62 KB `JobsView` bury the core triage→tailor→apply flow. Decided (with Ishani) to keep *all* features but re-hierarchy the UI so the funnel is the primary surface and everything else demotes to a "Tools" group — a UX change, not a feature cut. Cleanup was aggressive on true dead weight only: the 861 committed generated resume PDFs were untracked + deleted and `output/resumes/` gitignored (local-only now, so committing generated artifacts bought nothing but bloat; tailoring regenerates them per run). The LinkedIn data export was **not** committed — it carries private messages, phone numbers, logins, and security challenges; only `Connections.csv` was extracted into a gitignored `data/linkedin/`, and `data/` is gitignored wholesale. `scripts/linkedin_outreach.py` (ranks connections into a referral/cold-outreach shortlist by H-1B-sponsor + live-application signal) was wired to that default path with a `make outreach-linkedin` target and verified end-to-end (2373 → 276 shortlist, 64 interview-stage).
