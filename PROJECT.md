...
**Latest (2026-09-17):** UI refocus **complete** (M26–M31). The tool now opens on a single **Pipeline** surface (funnel + follow-ups + stage rail in one place); all other features demoted to a "Tools" group — nothing removed. Repo cleaned (fresh `output/resumes`, LinkedIn outreach wired), `JobsView` split 62 KB → 18.7 KB, an inline Prep shortcut added, and a long-standing intermittent `/api/stats` 500 fixed. 237 tests pass; verified end-to-end in-browser.
