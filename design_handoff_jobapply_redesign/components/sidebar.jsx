...
function Sidebar({ screen, setScreen, dark, setDark }) {
  const T = dark ? DARK : LIGHT;
  const stats = window.STATS;

  const navItems = [
    { id: "jobs",      icon: "⬡", label: "Jobs",      badge: stats.ready },
  ...
  const pipeline = [
    ...
    { label: "OA",        value: stats.oa,        color: "#F59E0B" },
    { label: "Interview", value: stats.interview,  color: "#EC4899" },
    { label: "Offer",     value: stats.offer,     color: "#10B981" },
  ];

  const submitted = stats.applied + stats.oa + stats.interview + stats.offer;
  const pct = Math.round((submitted / stats.total) * 100);
  ...
  return (
    <div style={{
        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setScreen(item.id)}
              ...
              {item.badge > 0 && (
                <span style={{ background: "#22C55E", color: "#fff", fontSize: 10, fontWeight: 800, borderRadius: 10, padding: "1px 7px", minWidth: 20, textAlign: "center" }}>
                  {item.badge}
                </span>
              )}
            </button>
      ...
      <div style={{ padding: "16px 20px", borderTop: `1px solid ${T.border}` }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>Pipeline</div>
        {pipeline.map(p => (
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ fontSize: 10, color: T.muted, marginTop: 4 }}>{submitted} of {stats.total} jobs</div>
        </div>
      </div>
      ...
      <div style={{ padding: "12px 20px", borderTop: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{
          width: 28, height: 28, borderRadius: "50%", background: `linear-gradient(135deg, ${T.accent}, #EC4899)`,
