...
Tasks:
- [x] `src/pipeline/interview_prep.py` — `generate_prep(job, jd_text, profile) -> dict` via `llm_client.complete`, strict no-fabrication system prompt (mirrors `resume_tailor`, reuses its `_load_profile`/`_profile_text`); shape-guards the JSON so the UI never crashes on a sparse response. Added `llm_client.get_provider_model()` to record which model produced a pack.
- [x] `interview_prep` table in `tracker.py` (`job_id` UNIQUE, `content` JSON, `model`, timestamps) + CRUD (`upsert_prep`, `get_prep`, `delete_prep`, `list_prep_jobs`).
- [x] Endpoints: `POST /api/jobs/{id}/prep` (generate + store; falls back to `jd_fetcher` when the stored description is thin), `GET /api/jobs/{id}/prep` (404 if none), `DELETE /api/jobs/{id}/prep`, `GET /api/prep` (interview-stage jobs + has_prep). 503/500 handling like `/tailor`.
- [x] `apps/web/src/components/PrepView.jsx` — a **"Interview Prep"** Sidebar section (5th nav) listing jobs at `oa`/`interview` with prep status + interview date; renders the full pack (snapshot / topics chips / three question buckets with talking points / questions-to-ask / checklist); Generate + Regenerate. Plus `App.jsx`/`Topbar`/`api.js` wiring.
- [~] JobDrawer "Prep" tab + focus-item deep-link — **deferred** to avoid touching the large `JobDrawer` component; the dedicated section (the actual ask) is delivered. Optional follow-up.
- [x] `tests/test_interview_prep.py` (7) — `generate_prep` with a mocked LLM (structure + default-fill + invalid-JSON→None), prep-table CRUD, `list_prep_jobs` filter, and generate/get/delete/404 endpoints.
- [x] Gate: `flake8` clean, `pytest tests/` → 90 passed, `npm run build` compiles (47 modules), live-server smoke test (prep list + 404-before-generate) green.
